...
func makeNoise(pixels []byte, freq, lacunarity, gain float32, octaves int, w, h int) {
	startTime := time.Now()
	...
	for i := 0; i < numRoutines; i++ {
		go func(i int) {
			...
			for j := start; j < end; j++ {
				x := j % w
				y := (j - x) / w
				noise[j] = turbulence(float32(x), float32(y), freq, lacunarity, gain, octaves)
...
func main() {
	...
	makeNoise(pixels, freq, lacunarity, gain, octaves, winWidth, winHeight)
